...
window.addEventListener('load', function() {
    fetch('/check-auth')
        .then(response => response.json())
        .then(data => {
            if (data.loggedin && window.location.pathname.endsWith('/cancer_nodejs')) {
...
function validateSession() {
  ...
  // ตรวจสอบทุกครั้งที่หน้าโหลดและเมื่อมีการกดปุ่มย้อนกลับ
  window.addEventListener('load', validateSession);
  window.addEventListener('pageshow', function(event) {
    if (event.persisted) { // ตรวจสอบว่าโหลดจาก cache หรือไม่
      validateSession();
    }
  });
